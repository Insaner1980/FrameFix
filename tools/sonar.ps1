...
Set-Content -LiteralPath $scanReport -Encoding utf8 -Value @(
    ...
    "Root: $repoRoot"
    "Project: $projectKey"
    "Command: reports/sonar.txt :: ./gradlew assembleDebug :app:debugUnitTestCoverage sonar"
...
try {
    ...
    if (-not $env:SONAR_TOKEN) {
        Add-Content -LiteralPath $scanReport -Encoding utf8 -Value @(
            ...
            "SonarQube CLI:n sonar auth login riittaa issueiden lukemiseen, mutta Gradle-skannaus tarvitsee SONAR_TOKEN-arvon."
            "Aseta analyysitoken ja aja esimerkiksi: `$env:SONAR_TOKEN=`"...`"; sonar"
            ""
        )
    ...
    & .\gradlew.bat "assembleDebug" ":app:debugUnitTestCoverage" "sonar" "--console=plain" *>&1 |
